...
// Operational DISPATCH control: which reps are responsible for working an account.
// Mirrors PropertyAssignments. Writes go to account_assignments; RLS enforces
// manager/admin-only writes and this UI gates on canManage too. It is a LABEL
// ONLY — being assigned grants no edit or read rights (permission-neutral).
...
export default function AccountAssignments({
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const [aaRes, ouRes] = await Promise.all([
        supabase.from("account_assignments").select("user_id").eq("account_id", accountId),
        supabase.from("org_users").select("user_id,full_name,email,role").order("full_name"),
      ]);
      if (cancelled) return;
      // Degrade gracefully if the table isn't live yet (deploy-before-migration).
      setAssignedIds(new Set(((aaRes.data ?? []) as { user_id: string }[]).map((r) => r.user_id)));
      setMembers(
  ...
  const assignedList = useMemo(
    () =>
      [...assignedIds]
        .map((id) => ({ userId: id, name: nameById.get(id) ?? id.slice(0, 8) }))
  ...
  async function assign(userId: string) {
    ...
    const { error } = await supabase
      ...
      .upsert(
        { account_id: accountId, user_id: userId, org_id: orgId, assigned_by: currentUserId },
