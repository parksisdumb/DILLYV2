...
-- 1) Backfill existing 'other' rows to 'visibility'
update public.touchpoints
  set engagement_phase = 'visibility'
  where engagement_phase = 'other';

-- 2) Replace check constraint
alter table public.touchpoints
  drop constraint if exists touchpoints_engagement_phase_check;

alter table public.touchpoints
  add constraint touchpoints_engagement_phase_check
  check (engagement_phase in ('first_touch', 'follow_up', 'visibility'));

-- 3) Recreate rpc_log_touchpoint with 'visibility' as valid phase + new default
...
-- 4) Recreate rpc_log_outreach_touchpoint with 'visibility' as valid phase
