-- Fix latent bug in rpc_convert_prospect surfaced by Focus Mode smoke test.
--
-- The function declared `v_result jsonb` and then did:
--   v_result := public.rpc_log_outreach_touchpoint(...);
--   v_touchpoint_id := (v_result->>'touchpoint_id')::uuid;
--
-- But rpc_log_outreach_touchpoint returns a TABLE, not jsonb. Assigning a
-- record-returning function to a jsonb variable raises:
--   invalid input syntax for type json: Token "(" is invalid
--
-- This was latent because the manager's prospect-convert flow always passes
-- p_log_touchpoint=false, so the bad code path never executed. Focus Mode
-- was the first caller to set p_log_touchpoint=true, which surfaced it.
--
-- Fix: drop the jsonb variable, capture the touchpoint_id directly via
-- `select ... into ... from rpc_log_outreach_touchpoint(...)`. Everything
...
begin
  ...
  --    Capture touchpoint_id directly from the table-returning function — the
  --    earlier `v_result jsonb := func()` pattern raised an "invalid input
  --    syntax for type json" error because plpgsql can't coerce a record into
  --    jsonb implicitly.
  if p_log_touchpoint and v_contact_id is not null and p_touchpoint_type_id is not null then
    select touchpoint_id
      ...
      from public.rpc_log_outreach_touchpoint(
  ...
  -- 6. Mark prospect as converted (no updated_at on prospects table)
  ...
  --    (updated_at handled by set_updated_at trigger)
