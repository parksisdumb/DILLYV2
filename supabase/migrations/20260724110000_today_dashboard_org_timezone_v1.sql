-- today_dashboard_org_timezone_v1
--
-- Timezone fix for the rep Today scoreboard. rpc_today_dashboard bucketed
-- "today" on the UTC calendar day: (happened_at at time zone 'utc')::date =
-- (now() at time zone 'utc')::date. For US orgs the UTC day starts at ~7pm local,
-- so evening-logged touchpoints counted toward the wrong day.
--
-- This re-creates the function IDENTICALLY except the six day-bucketing
-- comparisons now use 'America/Chicago' (the app default timezone; matches
-- APP_TIME_ZONE in src/lib/time.ts). Lift to a per-org timezone column later by
-- resolving the org's tz here instead of the hardcoded literal.
--
-- NOTE: must be applied to prod manually (prod `db push` is blocked for the local
-- CLI account). Pure CREATE OR REPLACE — no data change, safe to re-run.
...
begin
  ...
  where se.org_id = v_org_id
    ...
    and (se.created_at at time zone 'America/Chicago')::date = (now() at time zone 'America/Chicago')::date;
  ...
  where na.org_id = v_org_id
    ...
    and na.status = 'open'
    and (na.due_at at time zone 'America/Chicago')::date = (now() at time zone 'America/Chicago')::date;
