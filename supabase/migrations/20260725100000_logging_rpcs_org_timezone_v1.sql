-- logging_rpcs_org_timezone_v1
--
-- Timezone fix for the logging RPCs' day-bucketing. rpc_log_outreach_touchpoint
-- and rpc_log_touchpoint set v_event_date := (now() at time zone 'utc')::date and
-- (for outreach) count "today" on the UTC calendar day. For US orgs the UTC day
-- starts ~7pm local, so an evening-Central touchpoint credited the NEXT day —
-- which can BREAK a daily streak for a rep who actually worked every day.
--
-- These two functions are reproduced VERBATIM from
-- 20260613100000_fix_scoring_wildcard_rules_backfill_v1 (their latest definition),
-- with the ONLY change being at time zone 'utc' -> 'America/Chicago' on the three
-- day-bucketing expressions (2 in outreach: v_event_date + outreach_count_today;
-- 1 in log_touchpoint: v_event_date). Matches APP_TIME_ZONE in src/lib/time.ts and
-- the already-applied rpc_today_dashboard tz fix. Lift to per-org tz later.
--
-- Pure create-or-replace on two existing functions (no new objects, no drops) —
-- safe against the prod migration-history drift. Grants are preserved by
-- create-or-replace but re-stated to match the source. Apply via the SQL editor.
