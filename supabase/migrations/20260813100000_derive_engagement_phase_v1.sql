-- derive_engagement_phase_v1
--
-- Structural fix: engagement_phase is now DERIVED at log time from history, never
-- taken from the caller. The old model set the phase from WHICH screen logged the
-- touch (Grow → first_touch, detail panels/Focus/Advance → follow_up, property
-- panel → follow_up by default), so an evening rep who logs site visits from the
-- property page had ~all touches mislabeled follow_up. That zeroed her First Touch
-- KPI and starved the top of the funnel.
--
-- Rule: an OUTREACH touch is first_touch iff the anchor (contact, or property when
-- contact-less) has ZERO prior REAL (scored) touchpoints in the org; otherwise
-- follow_up. Non-outreach touches (bids, meetings) and passive-presence touches
-- (synced email / inbound — separate RPCs, untouched) keep 'visibility'; that
-- distinction is not derivable from history.
--
-- Both functions are reproduced VERBATIM from
-- 20260725100000_logging_rpcs_org_timezone_v1 (their latest definition), with the
-- ONLY change being a derivation block inserted immediately before each INSERT
...
declare
  ...
  v_points int := 0;
  v_event_date date := (coalesce(p_happened_at, now()) at time zone 'America/Chicago')::date;
  v_outreach_count int := 0;
  v_target numeric := 20;
  -- p_engagement_phase is IGNORED for outreach — the phase is derived below.
  v_engagement_phase text := 'first_touch';
begin
  ...
  -- DERIVE the phase from history, never from the caller: the first real (scored)
  -- touch on this contact is a first_touch; any prior real touch makes this a
  -- follow_up. Evaluated before the INSERT, so it excludes the row being logged.
  if exists (
    select 1
    from public.touchpoints t
    join public.score_events se on se.touchpoint_id = t.id
    where t.org_id = v_org_id
      and t.contact_id = p_contact_id
  ) then
    v_engagement_phase := 'follow_up';
  else
    v_engagement_phase := 'first_touch';
  end if;
  ...
  where t.org_id = v_org_id
    ...
    and (t.happened_at at time zone 'America/Chicago')::date = (now() at time zone 'America/Chicago')::date;
...
declare
  ...
  -- For non-outreach touches the passed phase stands (visibility by default);
  -- outreach touches are re-derived below.
...
begin
  ...
  -- DERIVE the phase for OUTREACH touches from history (anchor: contact, else
  -- property). Non-outreach touches keep the passed phase (visibility default) —
  -- a bid/meeting/drive-by isn't a first-vs-follow question.
  if v_type_is_outreach then
    if exists (
      select 1
      from public.touchpoints t
      join public.score_events se on se.touchpoint_id = t.id
      where t.org_id = v_org_id
        and (
          (p_contact_id is not null and t.contact_id = p_contact_id)
          or (p_contact_id is null and t.property_id = p_property_id)
        )
    ) then
      v_engagement_phase := 'follow_up';
    else
